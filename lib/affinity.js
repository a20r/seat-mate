...
function freshPair() {
  return { votes: 0, sumRight: 0, affinity: 0, skips: 0 };
}

// A rater hit "not sure" on this pair — record it so we stop pestering them
// about a pairing nobody can judge.
export function recordSkip(pairs, a, b) {
...
export function queryValue(pair) {
  const skips = pair.skips || 0;
  if (pair.votes === 0 && skips === 0) return 10; // always grab at least one observation
  const p = sigmoid(pair.affinity);
  const variance = p * (1 - p); // 0..0.25, peaks at total disagreement
  const base = (0.15 + variance) / (1 + pair.votes);
  // Each skip sharply lowers how much we want to ask about this pair again.
  return base / (1 + 3 * skips);
}
...
// adjacency bonus) but reports the plain affinity score for display. `locked`
// positions (manual placements) are never moved.
function optimizeTable(pairs, arr, adjSet, locked) {
...
// Arrange one table where some seats are pinned by manual placement. Locked
// seats stay put; the free guests are slotted into the remaining seats and
// optimised around the pinned ones.
function arrangeFixedTable(pairs, freeIds, fixedMap, k, adjSet) {
  const spt = k * 2;
  const arr = Array(spt).fill(null);
  const locked = Array(spt).fill(false);
  const idxOf = (side, pos) => (side === 'A' ? pos : k + pos);
  for (const [key, gid] of Object.entries(fixedMap)) {
    const [side, posStr] = key.split(':');
    const i = idxOf(side, Number(posStr));
    if (i >= 0 && i < spt) { arr[i] = gid; locked[i] = true; }
  }
  let fi = 0;
  for (let i = 0; i < spt; i++) {
    if (locked[i]) continue;
    arr[i] = freeIds[fi++] || null;
  }
  const { arr: best, score } = optimizeTable(pairs, arr, adjSet, locked);
  return { arr: best, locked, score };
}
...
// `caps[t]` is the number of *free* seats available on table t (total seats
// minus any manually-placed guests already sitting there).
function partitionOnce(pairs, clusters, caps) {
  const numTables = caps.length;
  const tables = Array.from({ length: numTables }, () => []);
  const remaining = (i) => caps[i] - tables[i].length;
  ...
  for (const cl of order) {
    ...
    for (let t = 0; t < numTables; t++) {
      if (tables[t].length + cl.length > caps[t]) continue;
    ...
    // table with the most space left so we still produce a usable chart.
    if (bestT === -1) bestT = tables.reduce((bi, _t, i) => (remaining(i) > remaining(bi) ? i : bi), 0);
...
export function bestMultiTableArrangement(pairs, guests, numTables, seatsPerTable, constraints = {}, placements = {}) {
  ...
  // Manual placements: seatKey "t:side:pos" -> guestId. These pin a guest to an
  // exact seat; everyone else is arranged around them.
  const tableFixed = Array.from({ length: numTables }, () => ({})); // t -> {"side:pos": gid}
  const placedIds = new Set();
  for (const [seatKey, gid] of Object.entries(placements || {})) {
    if (!present.has(gid) || placedIds.has(gid)) continue;
    const parts = String(seatKey).split(':');
    if (parts.length !== 3) continue;
    const t = Number(parts[0]); const side = parts[1]; const pos = Number(parts[2]);
    if (!Number.isInteger(t) || t < 0 || t >= numTables) continue;
    if (side !== 'A' && side !== 'B') continue;
    if (!Number.isInteger(pos) || pos < 0 || pos >= k) continue;
    const sk = `${side}:${pos}`;
    if (tableFixed[t][sk]) continue; // seat already taken
    tableFixed[t][sk] = gid;
    placedIds.add(gid);
  }

  // Cluster + partition only the un-placed guests, into the seats each table has
  // left after its manual placements.
  const freeIds = ids.filter((id) => !placedIds.has(id));
  const clusters = buildClusters(freeIds, cons);
  const caps = tableFixed.map((tf) => spt - Object.keys(tf).length);
  const partition = partitionClusters(pairs, clusters, caps);

  let totalScore = 0;
  const toGuest = (id) => (id ? (byId.get(id) || null) : null);
  const tables = partition.map((tblIds, t) => {
    const fixedMap = tableFixed[t];
    const { arr, locked, score } = arrangeFixedTable(pairs, tblIds, fixedMap, k, adjSet);
    totalScore += score;
    return {
      sideA: arr.slice(0, k).map(toGuest),
      sideB: arr.slice(k).map(toGuest),
      lockedA: locked.slice(0, k),
      lockedB: locked.slice(k),
      score,
    };
  });
